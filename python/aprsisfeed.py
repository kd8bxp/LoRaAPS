...
def callback(packet):
    ...
    if pto == 'N0CALL' and pformat == 'message': #put the callsign from APRS you want the messages forward from in pto ==
       ...
       msg = '{} {} {}'.format('N0CALL-02',pfrom,packet['message_text']).encode('utf-8') #put the callsign of the LoRaAPS board you wish to recieve your message at (remember 2 digit SSID)
       print("Sending message...")
       print(msg)
       ser = serial.Serial('/dev/ttyUSB0', 9600) #You may need to change the serial port here, the speed should be the same 
       ser.write(msg)
       ser.write(b'\n\r')
       ser.close()
