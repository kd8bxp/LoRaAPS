...
# This is a modified version of the LoraHam-Console from Travis Goodspeed's loraham project https://github.com/travisgoodspeed/loraham
# it now sends packets and will send a packet to another LoRaMaDor node. 
# this modified version removes most of the buttons, and moves the send button to below the message, making it a little easier to use this as a "chat". It will also show the verbose output of your board.
# To use this you will need a LoRaMaDor board hooked to your computer. 
# Aug 18, 2020 LeRoy Miller
...
# Sept 16, 2020 - LeRoy Miller, slight change in baudrate for LoRaAPS project.

import urwid, serial;


MYCALL = "N0CAL-02" #Change to your callsign (Remember 2 digit SSID)
...
ser = serial.Serial('/dev/ttyUSB0', 9600); #You may need to change your serial port
...
def on_send(button):
    ...
    packet_to_send = '{} {}'.format(dest.get_edit_text(),packet.get_edit_text()).encode(ENCODING)
...
buttons=urwid.Columns([sendpacket]);

pile=urwid.Pile([received,div,dest,packet,buttons]);
